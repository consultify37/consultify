...
const ImageComponent = ({ image, setImage, style='', title='Poza' }: Props) => {
  return (
    <div>
      <h2 className='text-[14px] font-semibold text-secondary mt-6 mb-2 ml-1'>{ title }</h2>
        { image ?
            <Image 
              ...
              height={411}
              alt='imagine'
              className={'rounded-xl h-[186px] w-[484px] ' + style}
            /> : 
            <div className={'flex justify-center items-center rounded-xl bg-white h-[186px] w-[484px] ' + style}>
              <input 
                type='file'
                id='slide-image'
                accept="image/*"
        ...
        { image ?
          <div className='flex flex-row justify-between items-center px-2 mt-4 mb-8'>
            <p className='text-secondary font-semibold'>{typeof image != 'string' ? image.name : '' }</p>
            <p 
              className='text-red-500 underline underline-offset-1 font-semibold cursor-pointer'
