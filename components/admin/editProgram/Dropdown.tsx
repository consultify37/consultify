        ...
        >
          {
            values.map((value, index) => (
              <div key={value+index} className='px-1'>
                <div
                  className='w-full rounded-xl bg-white px-5 py-3 cursor-pointer group'
                  onClick={() => {
                    setToggle(false)
                    setSelectedValue(value)
                  }}
                >
                  <p className='text-secondary group-hover:text-primary'>{value}</p>
                </div>
              </div>
            ))
          }
