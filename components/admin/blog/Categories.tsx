...
import { ArticleCategory, Category } from '../../../types'
...
const Categories = ({ categories, setCategories, selectedCategory, setSelectedCategory }: Props) => {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const response = await addDoc(collection(db, 'blog-categories'), { category: newCategory })

      setCategories(categories => [{ id: response.id, category: newCategory }, ...categories])
      setNewCategory('')
    } catch (e) {
      console.log(e)
      toast.error('Adaugarea categoriei a eșuat. Încearcă din nou.')
    }
    ...
    <div className='relative '>
      ...
      { isLoading ?
        <div className='absolute top-[18px] right-6 flex items-center'>
          <ReactLoading type="spin" color="#8717F8" width={16} height={16} />
        </div> :
        <div
      ...
      { categories.map((category) => (
        <div className='relative w-full p-4 py-2 flex flex-row justify-between items-center' key={category.id}>
          <div className='flex flex-row items-center cursor-pointer' 
            onClick={() => setSelectedCategory(category.category)}
          >
            <div 
              className='w-4 h-4 border-secondary border-[1.5px] rounded-[4px] mr-2'
              style={{ background: category.category == selectedCategory ? '#8717F8' : 'transparent' }}
            ></div>
            <p className='text-secondary font-semibold pr-8 pt-[2px] hidden-new-line'>{category.category}</p>
          </div>
          <div 
            className='absolute cursor-pointer hover:scale-[1.05] transition-all top-3 right-6'
