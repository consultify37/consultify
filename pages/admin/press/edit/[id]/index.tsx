...
const UpdateRealease = ({ categories, realease }: Props) => {
  ...
  const handleUpload = async (e: any) => {
    ...
    try {
      ...
      try {
        let results = imagesToBeDeleted.map((image) => {
          return new Promise(async (resolve, reject) => {
            try {
              image.fileName && await deleteObject(ref(storage, image.fileName))
              resolve({})
            } catch {
              reject({})
            }
      ...
      if ( newFileSnapshot ) {
        await updateDoc(doc(db, 'press-releases', realease.id), {
          ...
          smallLogos,
          file: {
            fileName: newFileSnapshot.ref.fullPath,
            url: newFileUrl
          }
        })
      } else {
      ...
      toast.success('Comunicatul de presă a fost actualizat cu succes!', { duration: 3000, style: { textAlign: 'center' }})
        ...
        <div className='mt-auto flex flex-row items-center justify-between w-full mb-8'>
          <div className='flex flex-row items-center'>
            ...
            { file && typeof file == 'string' ?
              <Link href={ realease.file.url } target='_blank'>
                <p className='text-[14px] text-[#00C572] font-semibold'>{realease.file.fileName}</p>
...
export const getServerSideProps = async (context: NextPageContext) => {
  ...
  const realease = { id: releaseSnap.id, ...data }
  ...
  return { props: { categories, realease }}
}